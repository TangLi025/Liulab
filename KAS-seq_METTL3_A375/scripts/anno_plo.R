...
CTRL_bed1 <- read.delim(paste0("~/KAS-METTL/",GROUP,"/06_macs2/peak_annotation/broad/",GROUP,"_Control_rep1_annotation_homer.xls"),header = T,)
CTRL_bed2 <- read.delim(paste0("~/KAS-METTL/",GROUP,"/06_macs2/peak_annotation/broad/",GROUP,"_Control_rep2_annotation_homer.xls"),header = T)

KO_bed1 <- read.delim(paste0("~/KAS-METTL/",GROUP,"/06_macs2/peak_annotation/broad/",GROUP,"_KDMETTL3_rep1_annotation_homer.xls"),header = T)
KO_bed2 <- read.delim(paste0("~/KAS-METTL/",GROUP,"/06_macs2/peak_annotation/broad/",GROUP,"_KDMETTL3_rep2_annotation_homer.xls"),header = T)
...
ggplot(plotdata, aes(x=`Genomic Regions`, y=`m6A peaks fraction`,fill=sample))+
  ...
  scale_fill_manual("",values = c('#969696','#BDBDBD', # #B3B3B3
                    ...
                    breaks=c('A375_Control_rep1','A375_Control_rep2','A375_KDMETTL3_rep1','A375_KDMETTL3_rep2'),
                    labels=c('WT-1','WT-2','METTL3_KO-1','METTL3_KO-2'))+
