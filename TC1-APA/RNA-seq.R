...
#generate bam list
sample_name <- c("TC1-AP_P0_rep1","TC1-AP_P0_rep2","TC1-AP_P7_rep1","TC1-AP_P7_rep2","TC1-AP_P10_rep1","TC1-AP_P10_rep2","TC1-AP_rP2o_rep1","TC1-AP_rP2o_rep2","TC1-AP_rP2n_rep1","TC1-AP_rP2n_rep2")
setwd("~/TC1_APA/04_bam_raw/")
dir <- getwd()
...
SE <- summarizeOverlaps(features = exonByGene,reads = bamlist,mode = 'Union',singleEnd = T,ignore.strand = T)
#add coldata
...
# get meta data
express_matrix <- countdata[,c(1,2,5,6)]
meta_data <- data.frame(rep = c("rep1","rep2","rep1","rep2"),treatment = c("P0","P0","P10","P10"),row.names = sample_name[c(1,2,5,6)],stringsAsFactors = TRUE)

# create DESeqDataSet
dds <- DESeqDataSetFromMatrix(countData = express_matrix,colData = meta_data,design = ~ treatment)

dds <- dds[rowSums(counts(dds)>=1)>=4,]
...
dds <- DESeq(dds)
resultsNames(dds)
res <- results(dds,name = "treatment_P10_vs_P0")
summary(res)
res <- res[!is.na(res$padj) & !is.na(res$log2FoldChange),]

res_select <- res[abs(res$log2FoldChange) > 1 & res$padj < 0.05,]
summary(res_select)
