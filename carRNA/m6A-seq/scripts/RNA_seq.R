...
#generate bam list
sample_name <- c("CTRL_input_rep1","CTRL_input_rep2","M3KO_input_rep1","M3KO_input_rep2")
setwd("~/carRNA_science/m6A-seq/04_bam_raw/")
dir <- getwd()
...
meta_data <- data.frame(rep = c("rep1","rep2","rep1","rep2"),treatment = c("CTRL","CTRL","M3KO","M3KO"),row.names = sample_name[1:4],stringsAsFactors = TRUE)
...
dds <- DESeq(dds)
resultsNames(dds)
res <- results(dds,name = "treatment_M3KO_vs_CTRL")
summary(res)
